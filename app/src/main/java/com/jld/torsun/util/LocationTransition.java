...
public class LocationTransition {
    ...
    public static LatLng LatLng_Transition(LatLng wgLoc) {
    ...
    public static Location Location_Transition(Location location) {
        ...
        dLat = (dLat * 180.0) / ((a * (1 - ee)) / (magic * sqrtMagic) * Math.PI);
        dLon = (dLon * 180.0) / (a / sqrtMagic * Math.cos(radLat) * Math.PI);
        location.setLatitude(location.getLatitude() + dLat);
        location.setLongitude(location.getLongitude() + dLon);
        return location;
    }
