...
import com.jld.torsun.adapter.Map_List_Adapter;
 ...
 * 百度地图“距离详情”页面
 */
public class MapMemberList extends BaseActivity implements View.OnClickListener, AdapterView.OnItemClickListener {
    ...
    private void initView() {
        ...
        listv_trouteam_list.setAdapter(new Map_List_Adapter(infos, this, latLng, imageLoader, isLoad));
        ...
        tv_title_title.setText(getResources().getString(R.string.map_nearby_distance) + "(" + (onlineNum + 1) + "/" + infos.size() + ")");
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        if ("0".equals(infos.get(position).getIsLoad())) {
    ...
    private void show_Call_Dialog(final int position) {
        ...
        confirm.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                String mobileNumber = infos.get(position).getPhone();
                if (!"无SIM卡".equals(mobileNumber)) {
                    dialog.dismiss();
                    Intent intent = new Intent(Intent.ACTION_CALL, Uri
                            .parse("tel:" + mobileNumber));
                    MapMemberList.this.startActivity(intent);
                } else {
                    dialog.dismiss();
                    ToastUtil.showToast(MapMemberList.this, R.string.team_no_sim, 2000);
