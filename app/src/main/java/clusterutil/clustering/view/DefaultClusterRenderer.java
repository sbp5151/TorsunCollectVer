...
import clusterutil.ui.SquareTextView;
...
public class DefaultClusterRenderer<T extends ClusterItem> implements
    ...
    public static Boolean isCluster;//是否聚合
    ...
    public List<Marker> ret_markers = new ArrayList<>();//保存所有标记
    public DefaultClusterRenderer(final Context context, BaiduMap map, ClusterManager<T> clusterManager) {
        mMap = map;
     ...
     */
    private Set<MarkerWithPosition> markersToRemove;
    ...
    private class RenderTask implements Runnable {
        ...
        public void run() {
            ...
            final Set<MarkerWithPosition> newMarkers = Collections.newSetFromMap(
                    new ConcurrentHashMap<MarkerWithPosition, Boolean>());
            LogUtil.d("ConcurrentHashMap","clusters.size():"+clusters.size());
    ...
    private class MarkerModifier extends Handler implements MessageQueue.IdleHandler {
        ...
        private void performNextTask() {
            if (!mOnScreenRemoveMarkerTasks.isEmpty()) {
                ...
                LogUtil.d("MapLocation", "mCreateMarkerTasks.poll()--");
        ...
        private void perform(MarkerModifier markerModifier) {
            ...
            if (!shouldRenderAsCluster(cluster)) {
                ...
                for (T item : cluster.getItems()) {
                    LogUtil.d("MapLocation", "添加覆盖物1:---"+"item:" + item.toString());
                    ...
                    if (marker == null) {
                        ...
                        info.setIsCluster(false);
                        Bundle bundle = new Bundle();
//                        bundle.putSerializable("info", info);
                        bundle.putParcelable("info", info);
                        marker.setExtraInfo(bundle);//将对象保存的marker里面
                        markerWithPosition = new MarkerWithPosition(marker);
                        mMarkerCache.put(item, marker);
                        ret_markers.add(marker);
                        if (animateFrom != null) {
            ...
            int num = 0;
            for (T item : items) {
                LogUtil.d("MapLocation", "添加覆盖物2:---"+"item:" + item.toString());

//                if (++num == items.size()) {
//                    LogUtil.d("MapLocation", "添加覆盖物2--" + item.getMarkerItem().getImg());
                    View item_view = LayoutInflater.from(context).inflate(R.layout.map_item_icon, null);
                    View rl_item_back = item_view.findViewById(R.id.rl_map_item_back);//覆盖物图片
                    RoundImageViewByXfermode2 item_icon = (RoundImageViewByXfermode2) item_view.findViewById(R.id.iv_map_item_icon);
                    TextView item_num = (TextView) item_view.findViewById(R.id.tv_map_item_num);
                    bitmapUtils.display(item_icon, item.getMarkerItem().getIcon());
                    item_num.setVisibility(View.VISIBLE);
                    item_num.setText(cluster.getSize() + "");
                    BitmapDescriptor bitmapDescriptor = BitmapDescriptorFactory.fromView(rl_item_back);
...
//                }
    ...
    private class AnimationTask extends AnimatorListenerAdapter implements ValueAnimator.AnimatorUpdateListener {
        ...
        private AnimationTask(MarkerWithPosition markerWithPosition, LatLng from, LatLng to) {
            ...
            LogUtil.d("onAnimationEnd", "创建onAnimationEnd");
        ...
        public void onAnimationEnd(Animator animation) {
            if (mRemoveOnComplete) {
                LogUtil.d("onAnimationEnd", "删除cluster" + markersToRemove.size());
